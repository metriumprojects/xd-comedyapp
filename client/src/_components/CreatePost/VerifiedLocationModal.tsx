          ...
          <View style={{
            ...
            >
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 8 }}>
                <Feather name="lock" size={16} color={COLORS.black} style={{ marginRight: 8 }} />
                <Text style={{ fontWeight: '500', fontSize: 16, color: COLORS.black }}>Add a verified location</Text>
              </View>
            ...
            <View style={{ flex: 1, paddingHorizontal: 20 }}>
                ...
                renderItem={({ item }) => {
                  if ('type' in item) {
                    ...
                    if (uiItem.type === 'header_nearby' || uiItem.type === 'header_passport') {
                      return <Text style={{ fontWeight: '700', fontSize: 14, color: COLORS.textPrimary, marginTop: (uiItem.type === 'header_passport' ? 18 : 0), marginBottom: 8 }}>{uiItem.label}</Text>;
                    }
                    if (uiItem.type === 'error_location') {
                      return <Text style={{ color: COLORS.textMuted, marginBottom: 12, textAlign: 'left' }}>Enable location permissions to see nearby verified places.</Text>;
                    }
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 15, fontWeight: '600', color: isSelected ? COLORS.primary : COLORS.textPrimary }}>{(item as any).name}</Text>
                        <Text style={{ color: COLORS.textSecondary, fontSize: 13, marginTop: 2 }}>{(item as any).address}</Text>
                      </View>
                      {isSelected && (
                        <Feather name="check" size={20} color={COLORS.primary} style={{ marginLeft: 10 }} />
                      )}
                    </TouchableOpacity>
                  );
                }}
                ListEmptyComponent={(!loadingVerifiedResults && verifiedResults.length === 0 && verifiedOptions.length === 0) ? <Text style={{ color: COLORS.textMuted, marginTop: 12, textAlign: 'center' }}>No results found</Text> : null}
              />
              ...
              <View style={{ 
                ...
                <View style={{ flexDirection: 'row', gap: 12 }}>
                  <TouchableOpacity
                    onPress={onClose}
                    style={{ backgroundColor: COLORS.black, borderRadius: 6, paddingHorizontal: 20, paddingVertical: 10 }}
                  >
                    <Text style={{ color: COLORS.textLight, fontWeight: '600', fontSize: 15 }}>Add</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    onPress={onClose}
                    style={{ backgroundColor: COLORS.primary, borderRadius: 8, paddingHorizontal: 20, paddingVertical: 10 }}
                  >
