...
export default function UsernameLoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    if (result.success && result.token && result.user) {
      try {
        const canonicalUserId = String(result.user.id || result.user._id || '');
        const firebaseUid = String(result.user.firebaseUid || result.user.uid || canonicalUserId);
        const avatarToStore = result.user.avatar || result.user.photoURL || result.user.profilePicture || '';
        ...
        console.error('Failed to write credentials to AsyncStorage:', storageErr);
        Alert.alert('Session Error', 'Could not save session to local storage. Please try again.');
      }
    } else {
