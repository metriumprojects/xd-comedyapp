...
export default function PasswordSignUpScreen() {
  ...
  const handleSignUp = async () => {
    ...
    setLoading(true);
    try {
      const result = await signUpUser(email, password, name || undefined, username, true);

      if (result.success) {
        if (result.needsVerification) {
          Alert.alert(
            "Verify your email",
            "We have sent a verification link to your email. Please check your inbox and verify your email before logging in.",
            [
              {
                text: "OK",
                onPress: () => {
