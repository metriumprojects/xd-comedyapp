  ...
  try {
    ...
    if (viewerVariants.length > 0) {
      ...
      try {
        ...
        const uniqueAuthorIds = Array.from(new Set(posts.map(post => {
          const p = post.toObject ? post.toObject() : post;
          return String(p.userId?._id || p.userId || p.authorData?._id || '');
        }).filter(Boolean)));
        
        const activeFollows = await Follow.find({
          followerId: { $in: viewerVariants },
          followingId: { $in: uniqueAuthorIds }
        }).select('followingId').lean();
        activeFollows.forEach(f => followingUserIds.add(String(f.followingId)));
      } catch (e) {
        console.warn('[enrich] Follow fetch failed:', e.message);
      }
    }
    ...
    return posts.map(post => {
      ...
      const processMediaItem = (item, fieldName) => {
        ...
        arr.forEach(m => {
          ...
          if (!seenUrls.has(url)) {
            ...
            const isVideo = lowerUrl.includes('.mp4') || lowerUrl.includes('.mov') || lowerUrl.includes('.m4v') || lowerUrl.includes('.webm') || lowerUrl.includes('video/upload') || lowerUrl.includes('/video/') || lowerUrl.includes('media/video') || m.type === 'video' || m.mediaType === 'video' || p.mediaType === 'video';
      ...
      p.laughCount = p.laughCount || (Array.isArray(p.laughedBy) ? p.laughedBy.length : 0);
      p.tomatoCount = p.tomatoCount || (Array.isArray(p.tomatoedBy) ? p.tomatoedBy.length : 0);
      p.viewsCount = p.viewsCount || 0;

      p.hasLaughed = false;
      if (viewerStrings.length > 0 && Array.isArray(p.laughedBy)) {
        p.hasLaughed = p.laughedBy.some(id => viewerStrings.includes(String(id)));
      }

      p.hasTomatoed = false;
      if (viewerStrings.length > 0 && Array.isArray(p.tomatoedBy)) {
        p.hasTomatoed = p.tomatoedBy.some(id => viewerStrings.includes(String(id)));
      }
      ...
      p.isFollowing = false;
      if (viewerStrings.length > 0) {
        p.isFollowing = followingUserIds.has(authorRef);
      }
