...
router.get('/', async (req, res) => {
  try {
    ...
    const { userId, limit, offset, cursor, cursorDate } = req.query;
    let viewerVariants = [];
    if (userId) {
      try {
    ...
    // 3. Execute optimized fetch with cursor support
    ...
    const lastPost = enrichedPosts.length > 0 ? enrichedPosts[enrichedPosts.length - 1] : null;
