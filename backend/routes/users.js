  ...
  try {
    ...
    if (requesterResolved && requesterResolved.canonicalId !== targetResolved.canonicalId) {
      ...
      promises.push(FollowRequest.findOne({
        fromUserId: { $in: requesterResolved.candidates.map(String) },
        toUserId: { $in: targetResolved.candidates.map(String) },
        status: 'pending'
      }).lean());
    } else {
    ...
    const [postCount, passportData, laughsResult, followDoc, followReqDoc] = await Promise.all(promises);
    ...
    const laughsCount = (laughsResult && laughsResult[0] && laughsResult[0].totalLaughs) || 0;
...
async function handleUpdateUser(req, res, authenticatedUserId) {
  try {
    ...
    const allowedFields = [
      ...
      'isPrivate', 'lastKnownLocation', 'pushToken', 'username', 'interests'
    ];
    ...
    if (body.phone !== undefined) {
      updateData.phoneNumber = body.phone;
    }
